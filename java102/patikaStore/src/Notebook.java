...
public class Notebook extends Database {
    ...
    public static void notebooks() {
        notebookList.add(new Notebook(1, 7000, 5, 50, "Huawei Matebook 14",
                "Huawei", "15.6 inç", "16 GB", "512"));
        notebookList.add(new Notebook(2, 5688, 10, 75, "LENOVO 14 IGL",
                "Lenovo", "14 inç", "8 GB", "1024"));
        notebookList.add(new Notebook(3, 688, 10, 75, "ASUS Tuf Gaming",
                "Asus", "15.6 inç", "32 GB", "2048"));
    }

    public static void notebookBrandForComparator() {
        notebookList2.clear();
        for (Notebook say : Notebook.notebookList) {
            notebookList2.put(new Notebook(say.getId(), say.getUnitPrice(), say.getDiscountRate(), say.getAmount(), say.getProductName(),
                    say.getBrand(), say.getScreenSize(), say.getRam(), say.getStorage()), say.getId());
            System.out.println(notebookList2.size() + " " + say.getId());
        }
        System.out.println("-------------------------------------------------------------");
        System.out.printf("%-4s %-30s %-13s %-11s %-15s %-10s  %-10s  %-10s %-15s %-1s", "| ID", "| URUN " + "ADI ", "|    FIYAT ",
        ...
        for (Notebook bay : notebookList2.keySet()) {
            System.out.printf("%-4s %-30s %-10s %-10s %-15s %-10s  %-10s  %-10s %-15s %-1s",
                    ...
                    "| " + bay.getRam(), "| " + bay.getAmount() + " Adet", "|   %" + bay.getDiscountRate(), "|\n");
        }
        System.out.println("-------------------------------------------------------------");
    }
    ...
    public static void notebookPrint() {
        System.out.println("-------------------------------------------------------------");
        System.out.printf("%-4s %-30s %-10s %-10s %-15s %-10s  %-10s  %-10s %-15s %-1s", "| ID", "| URUN " + "ADI ", "|    FIYAT ",
                "| " + "  MARKA ", "|    DEPOLAMA ", "|  EKRAN ", "|   RAM ", "|   STOK ", "| INDIRIM " + "ORANI", "|");
        System.out.println();
        for (Notebook say : Notebook.notebookList) {
            System.out.printf("%-4s %-30s %-10s %-10s %-15s %-10s  %-10s  %-10s %-15s %-1s",
                    ...
                    "| " + say.getRam(), "| " + say.getAmount() + " Adet", "|   %" + say.getDiscountRate(), "|\n");
        }
    }
    ...
    public static void notebooksAdd(int i, int unitPrice, int discountRate, int amount, String productName,
                                    String brand, String screenSize, String ram, String storage){
        notebookList.add(new Notebook(i, unitPrice, discountRate, amount, productName, brand, screenSize, ram, storage));
    }
